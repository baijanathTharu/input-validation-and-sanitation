...
describe('testing email field', () => {
  ...
  it('"test@test.com" is a valid email', async () => {
    const isValid = await emailSchema.isValid({ email: 'test_abc.user@test.com' });
    expect(isValid).toBe(true);
  });
...
describe('testing company name field', () => {
  it(`"John & Doe Company" is a valid name`,async () => {
    const isValid = await companyNameSchema.isValid({ name: 'John & Doe Company' });
    expect(isValid).toBe(true);
  })
  it(`"John's Doe Company" is a valid name`,async () => {
    const isValid = await companyNameSchema.isValid({ name: 'John\'s Doe Company' });
    expect(isValid).toBe(true);
  })
  ...
  it(`"Some Company With Very Long Name" is an invalid name`,async () => {
    const isValid = await companyNameSchema.isValid({ name: 'Some Company With Very Long Name' });
    expect(isValid).toBe(false);
  })
})
...
describe('testing url field', () => {
  ...
  it(`"https://www.test-site.com" is a valid url`, async()=> {
    const isValid = await urlSchema.isValid({ url: 'https://www.test-site.com' });
    expect(isValid).toBe(true);
  })
  ...
  it(`"javascript:alert('Don't laugh, this is not a joke!')"  has invalid protocal`, async()=> {
    const isValid = await urlSchema.isValid({ url: "javascript:alert('Don't laugh, this is not a joke!')"});
    expect(isValid).toBe(false);
  })
})
